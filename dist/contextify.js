...
var global = exports;
var local = host.Object.create(null);
local.Object = Object;
local.Array = Array;
...
var FunctionBind = uncurryThis(Function.prototype.bind);
...
var DEBUG = false;
var OPNA = "Operation not allowed on contextified object.";
...
var Contextified = new host.WeakMap();
var Decontextified = new host.WeakMap();
var ObjectHasInstance = uncurryThis(local.Object[Symbol.hasInstance]);
...
var SHARED_OBJECT = { __proto__: null };
...
function doPreventExtensions(target, object, doProxy) {
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    if (!desc)
      continue;
    if (!local.Reflect.setPrototypeOf(desc, null))
      unexpected();
    ...
    if (!local.Reflect.defineProperty(target, key, desc))
      unexpected();
  }
  if (!local.Reflect.preventExtensions(target))
    unexpected();
}
var Decontextify = host.Object.create(null);
Decontextify.proxies = new host.WeakMap();
Decontextify.arguments = (args) => {
  if (!host.Array.isArray(args))
...
Decontextify.instance = (instance, klass, deepTraps, flags, toStringTag) => {
  if (typeof instance === "function")
    return Decontextify.function(instance);
  const base = host.Object.create(null);
  base.get = (target, key, receiver) => {
    try {
      if (key === "vmProxyTarget" && DEBUG)
        return instance;
      if (key === "isVMProxy")
        return true;
      if (key === "constructor")
    ...
    if (key === host.Symbol.toStringTag && toStringTag)
      return toStringTag;
    try {
...
Decontextify.object = (object, traps, deepTraps, flags, mock) => {
  ...
  base.getOwnPropertyDescriptor = (target, prop) => {
    ...
    if (!def) {
      ...
      desc = {
        __proto__: null,
        get: Decontextify.value(def.get) || void 0,
        set: Decontextify.value(def.set) || void 0,
  ...
  base.defineProperty = (target, key, descriptor) => {
    ...
    if (descriptor.get || descriptor.set) {
      propertyDescriptor.get = Contextify.value(descriptor.get, null, deepTraps, flags) || void 0;
      propertyDescriptor.set = Contextify.value(descriptor.set, null, deepTraps, flags) || void 0;
  ...
  if (host.Array.isArray(object)) {
    ...
    base.get = (target, key, receiver) => {
      if (key === host.Symbol.toStringTag)
        return;
...
var Contextify = host.Object.create(null);
...
Contextify.instance = (instance, klass, deepTraps, flags, toStringTag) => {
  if (typeof instance === "function")
    return Contextify.function(instance);
  const base = host.Object.create(null);
  base.get = (target, key, receiver) => {
    try {
      if (key === "vmProxyTarget" && DEBUG)
...
Contextify.function = (fnc, traps, deepTraps, flags, mock) => {
  ...
  let proxy;
  base.apply = (target, context, args) => {
    if (host.version < 8 && fnc === host.Buffer && typeof args[0] === "number") {
...
Contextify.object = (object, traps, deepTraps, flags, mock) => {
  ...
  base.set = (target, key, value, receiver) => {
    if (key === "__proto__")
      return false;
    if (flags && flags.protected && typeof value === "function")
      return false;
  ...
  base.getOwnPropertyDescriptor = (target, prop) => {
    ...
    if (!def) {
      ...
      desc = {
        __proto__: null,
        get: Contextify.value(def.get, null, deepTraps, flags) || void 0,
        set: Contextify.value(def.set, null, deepTraps, flags) || void 0,
        enumerable: def.enumerable === true,
        configurable: def.configurable === true
      };
    } else {
  ...
  base.defineProperty = (target, key, descriptor) => {
    ...
    if (flags && flags.protected) {
      if (descGet || descSet || typeof descValue === "function")
        return false;
    }
    const propertyDescriptor = host.Object.create(null);
    if (descGet || descSet) {
      propertyDescriptor.get = Decontextify.value(descGet, null, deepTraps, flags) || void 0;
      propertyDescriptor.set = Decontextify.value(descSet, null, deepTraps, flags) || void 0;
      propertyDescriptor.enumerable = descriptor.enumerable === true;
      propertyDescriptor.configurable = descriptor.configurable === true;
    } else {
...
var BufferMock = host.Object.create(null);
...
var BufferOverride = host.Object.create(null);
...
var LocalBuffer = global.Buffer = Contextify.readonly(host.Buffer, BufferMock);
...
var oldPrepareStackTraceDesc = Reflect.getOwnPropertyDescriptor(Error, "prepareStackTrace");
var currentPrepareStackTrace = Error.prepareStackTrace;
var wrappedPrepareStackTrace = new host.WeakMap();
if (typeof currentPrepareStackTrace === "function") {
  wrappedPrepareStackTrace.set(currentPrepareStackTrace, currentPrepareStackTrace);
}
var OriginalCallSite;
Error.prepareStackTrace = (e, sst) => {
  OriginalCallSite = sst[0].constructor;
};
new Error().stack;
if (typeof OriginalCallSite === "function") {
  let makeCallSiteGetters = function(list) {
    const callSiteGetters2 = [];
    ...
    return callSiteGetters2;
  }, applyCallSiteGetters = function(callSite, getters) {
...
var exportsMap = host.Object.create(null);
